...
class telmarh_testimonials extends WP_Widget {



// constructor

    function telmarh_testimonials() {

		$widget_ops = array('classname' => 'telmarh_testimonials_widget', 'description' => __( 'What are people saying? Let everyone know.', 'telmarh') );

        parent::__construct(false, $name = __('Home Page Testimonials', 'telmarh'), $widget_ops);

		$this->alt_option_name = 'telmarh_testimonials_widget';
	...
	<p><?php _e('In order to display this widget, you must first add some Testimonial custom post types.', 'telmarh'); ?></p>
	...
	<p><label for="<?php echo sanitize_text_field( $this->get_field_id( 'number' )); ?>"><?php _e( 'Number of testimonials to show (-1 shows all):', 'telmarh' ); ?></label>
    ...
    <p><label for="<?php echo sanitize_text_field( $this->get_field_id('see_all')); ?>"><?php _e('<em>Optional:</em> Enter the URL for your Testimonials page.', 'telmarh'); ?></label>
	...
	<label for="<?php echo sanitize_text_field( $this->get_field_id( 'random' )); ?>"><?php _e( 'Show random testimonials?', 'telmarh' ); ?></label></p>
	...
	function update($new_instance, $old_instance) {
		...
		if ( isset($alloptions['telmarh_testimonials']) )

			delete_option('telmarh_testimonials');
	...
	function flush_widget_cache() {
		...
		wp_cache_delete('telmarh_testimonials', 'widget');
	...
	function widget($args, $instance) {
		...
		if ( ! $this->is_preview() ) {
			...
			$cache = wp_cache_get( 'telmarh_testimonials', 'widget' );
		...
		$title = ( ! empty( $instance['title'] ) ) ? esc_attr( $instance['title'] ) : __( 'Testimonials', 'telmarh' );
...
?>
	...
	<?php
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'telmarh_testimonials', $cache, 'widget' );
