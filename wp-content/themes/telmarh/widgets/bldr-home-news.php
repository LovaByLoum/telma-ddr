...
class telmarh_home_news extends WP_Widget {



// constructor

    function telmarh_home_news() {

		$widget_ops = array('classname' => 'telmarh_home_news_widget', 'description' => __( 'Show all your news posts on your home page.', 'telmarh') );

        parent::__construct(false, $name = __('Home Page News Posts', 'telmarh'), $widget_ops);

		$this->alt_option_name = 'telmarh_home_news_widget';
	...
	<p><label for="<?php echo sanitize_text_field( $this->get_field_id( 'category' )); ?>"><?php _e( 'Enter the slug for your category or leave empty to show posts from all categories.', 'telmarh' ); ?></label>
    ...
    <p><label for="<?php echo sanitize_text_field( $this->get_field_id('see_all_text')); ?>"><?php _e('Button Text. Default is set to <em>See All</em>.', 'telmarh'); ?></label>
	...
	function update($new_instance, $old_instance) {
		...
		if ( isset($alloptions['telmarh_home_news']) )

			delete_option('telmarh_home_news');
	...
	function flush_widget_cache() {
		...
		wp_cache_delete('telmarh_home_news', 'widget');
	...
	function widget($args, $instance) {
		...
		if ( ! $this->is_preview() ) {
			...
			$cache = wp_cache_get( 'telmarh_home_news', 'widget' );
		...
		$title = ( ! empty( $instance['title'] ) ) ? esc_html( $instance['title'] ) : __( 'News', 'telmarh' );
		...
		$see_all_text = isset( $instance['see_all_text'] ) ? esc_html($instance['see_all_text']) : __( 'See All', 'telmarh' );
...
?>
            	...
            	<?php while ( $mt->have_posts() ) : $mt->the_post(); ?>
                	...
                	<div class="col-1-3 tri-clear">
            			<div class="grid-block fade">
            				...
            				<a href="<?php the_permalink(); ?>"><?php the_post_thumbnail('telmarh-home-blog'); ?></a>
                        </div>
							<h5><?php the_title(); ?></h5>
                        	<p><?php $content = get_the_content(); echo wp_trim_words( $content , '20' ); ?> <a href="<?php the_permalink(); ?>">
                             <?php __( 'Read More', 'telmarh' ); ?></a></p>
                    </div><!-- col-1-3 -->
	...
	<?php
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'telmarh_home_news', $cache, 'widget' );
