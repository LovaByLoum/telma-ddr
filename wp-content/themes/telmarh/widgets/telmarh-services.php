...
class telmarh_services extends WP_Widget {



// constructor

    function telmarh_services() {

		$widget_ops = array('classname' => 'telmarh_services_widget', 'description' => __( 'Have a lot of services? Show them to the world.', 'telmarh') );

        parent::__construct(false, $name = __('Home Page Services', 'telmarh'), $widget_ops);

		$this->alt_option_name = 'telmarh_services_widget';
	...
	<p><?php _e('In order to display this widget, you must first add some Services custom post types.', 'telmarh'); ?></p>
	...
	<p><label for="<?php echo sanitize_text_field( $this->get_field_id( 'number' )); ?>"><?php _e( 'Number of Services to show (-1 shows all):', 'telmarh' ); ?></label>
    ...
    <p><label for="<?php echo sanitize_text_field( $this->get_field_id('see_all')); ?>"><?php _e('<em>Optional:</em> Enter the URL for your Services page.', 'telmarh'); ?></label>
	...
	function update($new_instance, $old_instance) {
		...
		if ( isset($alloptions['telmarh_services']) )

			delete_option('telmarh_services');
	...
	function flush_widget_cache() {
		...
		wp_cache_delete('telmarh_services', 'widget');
	...
	function widget($args, $instance) {
		...
		if ( ! $this->is_preview() ) {
			...
			$cache = wp_cache_get( 'telmarh_services', 'widget' );
		...
		$title = ( ! empty( $instance['title'] ) ) ? esc_attr( $instance['title'] ) : __( 'Services', 'telmarh' );
...
?>
	...
	<?php
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'telmarh_services', $cache, 'widget' );
