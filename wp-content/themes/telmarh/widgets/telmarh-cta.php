...
class telmarh_Action extends WP_Widget {



// constructor

    function telmarh_action() {

		$widget_ops = array('classname' => 'telmarh_action_widget', 'description' => __( 'Create a widget area for your home page.', 'telmarh') );

        parent::__construct(false, $name = __('Home Page Widget Call-to-Action', 'telmarh'), $widget_ops);

		$this->alt_option_name = 'telmarh_action_widget';
	...
	<label for="<?php echo sanitize_text_field( $this->get_field_id('action_text')); ?>"><?php _e('Enter your call to action.', 'telmarh'); ?></label>
	...
	<label for="<?php echo sanitize_text_field( $this->get_field_id('action_btn_link')); ?>"><?php _e('Link for the button', 'telmarh'); ?></label>
	...
	<label for="<?php echo sanitize_text_field( $this->get_field_id('action_btn_text')); ?>"><?php _e('Title for the button', 'telmarh'); ?></label>
	...
	function update($new_instance, $old_instance) {
		...
		if ( isset($alloptions['telmarh_action']) )

			delete_option('telmarh_action');
	...
	function flush_widget_cache() {
		...
		wp_cache_delete('telmarh_action', 'widget');
	...
	function widget($args, $instance) {
		...
		if ( ! $this->is_preview() ) {
			...
			$cache = wp_cache_get( 'telmarh_action', 'widget' );
...
?>
	...
	<?php
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'telmarh_action', $cache, 'widget' );
