...
class telmarh_Contact_Info extends WP_Widget {



// constructor

    function telmarh_contact_info() {

		$widget_ops = array('classname' => 'telmarh_contact_info_widget', 'description' => __( 'Display your contact info', 'telmarh') );

        parent::__construct(false, $name = __('MT: Contact Info Widget', 'telmarh'), $widget_ops);

		$this->alt_option_name = 'telmarh_contact_info';
	...
	<p><label for="<?php echo sanitize_text_field( $this->get_field_id( 'address' )); ?>"><?php _e( 'Enter your address', 'telmarh' ); ?></label>
	...
	<p><label for="<?php echo sanitize_text_field( $this->get_field_id( 'phone' )); ?>"><?php _e( 'Enter your phone number', 'telmarh' ); ?></label>
	...
	<p><label for="<?php echo sanitize_text_field( $this->get_field_id( 'email' )); ?>"><?php _e( 'Enter your email address', 'telmarh' ); ?></label>
	...
	function update($new_instance, $old_instance) {
		...
		if ( isset($alloptions['telmarh_contact_info']) )

			delete_option('telmarh_contact_info');
	...
	function flush_widget_cache() {
		...
		wp_cache_delete('telmarh_contact_info', 'widget');
	...
	function widget($args, $instance) {
		...
		if ( ! $this->is_preview() ) {
			...
			$cache = wp_cache_get( 'telmarh_contact_info', 'widget' );
		...
		$title = ( ! empty( $instance['title'] ) ) ? esc_attr( $instance['title'] ) : __( 'Contact info', 'telmarh' );
		...
		if( ($address) ) {
			...
			echo '<span>' . __('Address: ', 'telmarh') . '</span>' . wp_kses_post( $address );
		...
		if( ($phone) ) {
			...
			echo '<span>' . __('Phone: ', 'telmarh') . '</span>' . wp_kses_post( $phone );
		...
		if( ($email) ) {
			...
			echo '<span>' . __('Email: ', 'telmarh') . '</span>' . '<a href="mailto:' . sanitize_email( $email ) . '">' . sanitize_email( $email ) . '</a>';
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'telmarh_contact_info', $cache, 'widget' );
