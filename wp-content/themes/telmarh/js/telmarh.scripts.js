...
jQuery(function ($) {
  ...
  if (jQuery("#page-loginform").length > 0) {
    jQuery("#page-loginform").validate({
      ...
      onkeyup: false,
      errorElement: "p",
      errorClass: "login-username-page error",
      errorPlacement: function (error, element) {
        error.insertBefore(".data-error");
      },
      rules: {
  ...
  //lost password
  if (jQuery("#page-lostpassword").length > 0) {
...
jQuery.validator.addMethod("loginNameEmailExist", function (value, element, arg) {
  var isSuccess = false;
  jQuery.ajax({
    type: "post",
    url: ajaxurl,
    async: false,
    data: {
      action: "telmarh_check_login_email_exist",
      data_type: value
    },
    success: function (response) {
      isSuccess = response ? false : true
    }
  });
  return isSuccess;
});
