...
class Spreadsheet_Excel_Reader {
	...
	function dump($row_numbers=false,$col_letters=false,$sheet=0,$table_class='excel') {
		...
		$out .= "<tbody>\n";
		for($row=1;$row<=$this->rowcount($sheet)-1;$row++) {
			...
			for($col=1;$col<=$this->colcount($sheet)-1;$col++) {
	...
	// DUMP AN HTML TABLE OF THE ENTIRE XLS DATA
	// =========================================
	function dump_clean($sheet=0, $startRow = 1, $startCol = 1, $table_class='excel') {
		$out = "<table class=\"$table_class\" cellspacing=0>";
		$out .= "<tbody>\n";
		...
		$out .= "<tr class='row-numero-colonne'>";
		for($col=$startCol;$col<=$endCol;$col++) {
			$out .= '<td style="min-width: 100px; text-align:center;background-color:'. (($col%2==0)?'#000080':'#0000FF') .';color:white;" class="numero-column">' . $col . "</td>";
		}
		$out .= "</tr>";
		
		$out .= "<tr class='row-colonne-slug'>";
		for($col=$startCol;$col<=$endCol;$col++) {
			$out .= '<td><select class="columnconfig" name="column_name[]"style="width: 100%;"><option value="aucun">Aucun</option>' .apply_filters("jie_column_list","<option>Aucun</option>",$col). "</select></td>";
		}
	...
	// GET THE ENTIRE XLS DATA
	// =========================================
	function get_data($sheet=0, $startRow = 1, 	$endRow = null, $startCol = 1) {
		$result = array(); 
		
		$lastRow = $this->rowcount($sheet);
		if(is_null($endRow) || $endRow > $lastRow){
			$endRow = $this->rowcount($sheet);
		}
			...
			for($col=$startCol;$col<=$endCol;$col++) {
				...
				if(!$this->sheets[$sheet]['cellsInfo'][$row][$col]['dontprint']) {
					...
					if(isset($_POST["column_name"])){
						$rowdata[$_POST["column_name"][$col-1]]= utf8_encode($val);
					}else{
						$rowdata[$col]= utf8_encode($val);
					}
				}
			}
			if(!$empty){
