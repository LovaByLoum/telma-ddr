...
if ( isset($loginlockdown_db_version) ) {
	...
	add_action('init', 'loginLockdown_install');
	...
	function ll_wp_authenticate_username_password($user, $username, $password) {
		...
		if ( !$userdata ) {
			return new WP_Error('invalid_username', '<strong>ERREUR</strong>');
		}
		...
		if ( !wp_check_password($password, $userdata->user_pass, $userdata->ID) ) {
			return new WP_Error('incorrect_password', '<strong>ERREUR</strong>');
		}
	...
	function wp_authenticate($username, $password) {
		...
		if ( "" != isLockedDown() ) {
			return new WP_Error('incorrect_password', "<strong>ERREUR</strong>: Désolé, mais cette plage d'adresses IP a été bloqué en raison d'un trop grand nombre de tentatives de connexion infructueuses.<br /><br />Veuillez réessayer plus tard.");
		}
		...
		if ( $user == null ) {
			...
			$user = new WP_Error('authentication_failed', __('<strong>ERREUR</strong>'));
		}
		...
		if (is_wp_error($user) && !in_array($user->get_error_code(), $ignore_codes) ) {
			...
			if ( $loginlockdownOptions['max_login_retries'] <= countFails($username) ) {
				lockDown($username);
				return new WP_Error('incorrect_password', __("<strong>ERREUR</strong>: Désolé, mais cette plage d'adresses IP a été bloqué en raison d'un trop grand nombre de tentatives de connexion infructueuses.<br /><br />Veuillez réessayer plus tard."));
			}
			if ( 'yes' == $loginlockdownOptions['mask_login_errors'] ) {
				return new WP_Error('authentication_failed', '<strong>ERREUR</strong>');
			} else {
